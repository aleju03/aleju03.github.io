...
export function CommandPalette() {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const actions: Item[] = [
      ...
      { id: 'alejos', group: t.palette.actions, label: t.palette.bootAlejOS, hint: t.palette.desktopMode, icon: <DesktopIcon size={16} />, run: () => window.dispatchEvent(new CustomEvent(BOOT_OS_EVENT, { detail: { via: 'palette' } })) },
      { id: 'pc', group: t.palette.actions, label: t.palette.bootPc, hint: t.palette.noThreeD, icon: <MonitorIcon size={16} />, run: () => window.dispatchEvent(new CustomEvent(BOOT_OS_EVENT, { detail: { flat: true, via: 'palette' } })) },
      { id: 'world', group: t.palette.actions, label: t.palette.walkWorld, hint: t.palette.loadTheWorld, icon: <PersonSimpleWalkIcon size={16} />, run: () => window.dispatchEvent(new CustomEvent(BOOT_OS_EVENT, { detail: { world: true, via: 'palette' } })) },
