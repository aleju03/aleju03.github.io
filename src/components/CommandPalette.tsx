...
export function CommandPalette() {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const actions: Item[] = [
      ...
      { id: 'pc', group: t.palette.actions, label: t.palette.bootPc, hint: t.palette.noThreeD, icon: <MonitorIcon size={16} />, run: () => window.dispatchEvent(new CustomEvent(BOOT_OS_EVENT, { detail: { flat: true } })) },
      { id: 'room', group: t.palette.actions, label: t.palette.walkRoom, hint: t.palette.skipTheBoot, icon: <PersonSimpleWalkIcon size={16} />, run: () => window.dispatchEvent(new CustomEvent(BOOT_OS_EVENT, { detail: { room: true } })) },
